  ...
  public String execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
    ...
    try {
      ...
      if (isDoublon) {
        ...
        return rechargerFormulaire(request, client, "Cette raison sociale existe déjà dans la base de données.");
      }
    } catch (Exception e) {
      log.error("Erreur lors de la vérification de l'unicité", e);
      return rechargerFormulaire(request, client, "Erreur technique lors de la vérification des données.");
    }
    ...
    if (!violations.isEmpty()) {
      ...
      log.warn("Échec Bean Validation pour le client {}: {}", client.getRaisonSociale(), messageErreur);
    ...
    try {
      ...
      session.setAttribute("successMessage", "Le client " + client.getRaisonSociale() + " a été enregistré avec succès.");

      log.info("Client {} enregistré avec succès (ID: {})", client.getRaisonSociale(), client.getId());
      ...
      return rechargerFormulaire(request, client, "Une erreur technique a empêché l'enregistrement en base de données.");
    }
  }

  /**
   * Méthode utilitaire pour renvoyer l'utilisateur au formulaire avec ses données pré-remplies en cas d'erreur.
   */
  private String rechargerFormulaire(HttpServletRequest request, Client client, String messageErreur) {
    // On replace l'objet client dans la requête pour le "Data-Binding" du JSP (conserve la saisie)
    ...
    request.setAttribute("pageTitle", (isEditMode ? "Modifier" : "Nouveau") + " Client - Reverso CRM");
